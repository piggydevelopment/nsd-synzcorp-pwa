  ...
  const handleSubmit = async () => {
    ...
    if (rating_star == 0 || rating1 == 0 || rating2 == 0) {
      ...
      try {
        ...
        await api.post("/api/evaluation", data);
  ...
  return (
    <Box sx={{ backgroundColor: "#FFF", paddingBottom: "80px" }}>
      ...
      <Box p={2}>
          ...
          <Stack spacing={2} direction="row" alignItems="center">
            ...
            <div>
              <Typography
                ...
                mb={1}
                component="div"
                sx={{ fontWeight: 600, fontSize: 16, color: "#2C2C2C" }}
              >
        ...
        {value !== null && (
          <Box mt={1} sx={{ textAlign: "center", color: "#F3B85D" }}>
            {labels[hover !== -1 ? hover : value]}
          </Box>
        )}
        <Box my={3} px={2}>
          <FormControl>
            <FormLabel className="NotoSansThai">
              ความพึงพอใจต่อ SynZ Application *
            </FormLabel>
            <RadioGroup
        ...
        <Box my={3} px={2}>
          <FormControl>
            <FormLabel className="NotoSansThai">
              ท่านมีความพึงพอใจที่องค์กร
              จัดให้มีบริการให้คำปรึกษาสุขภาพใจออนไลน์สำหรับผู้ปฏิบัติงานที่ระดับใด*
            </FormLabel>
            <RadioGroup
              aria-labelledby="demo-radio-buttons-group-label"
              defaultValue="0"
